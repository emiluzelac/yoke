...
import { normalizeTimeoutMs } from './client/transport';
...
/**
 * Socket inactivity limit for chat requests, in milliseconds. `0` disables it.
 *
 * A self-hosted server can prefill a long conversation for many minutes before
 * emitting its first byte, so this is deliberately generous by default.
 */
export function getRequestTimeoutMs(): number {
	const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
	return normalizeTimeoutMs(config.get<number>('requestTimeoutMs'));
}
...
/**
 * Migrate the legacy boolean `yoke.debug` setting to `debugMode`.
 *

...
const zh: Translations = {
	...
	// API Key
	'auth.apiKeyRequiredDetail': '请先配置 API Key 或自托管地址',
	'auth.prompt': '请输入 API Key。仅在使用官方 DeepSeek API 时需要；自托管服务通常无需填写。',
	'auth.placeholder': 'sk-... 或服务商令牌',
	'auth.emptyValidation': 'API Key 不能为空',
	'auth.saved': 'API Key 已安全保存。',
	'auth.removed': 'API Key 已移除。',
	'auth.notConfigured':
		'未配置 API Key。请运行 "Yoke: 设置 API Key"，或将 yoke.baseUrl 指向自托管服务。',
	...
	'notice.toolDrift':
		'⚠️ 工具列表不稳定，缓存命中率可能下降。[了解更多](https://github.com/emiluzelac/yoke/blob/main/docs/notices/tool-drift.zh.md)',
...
const en: Translations = {
	...
	// API Key
	'auth.apiKeyRequiredDetail': 'Run Yoke: Set API Key, or point yoke.baseUrl at your own server.',
	'auth.prompt':
	...
	'auth.notConfigured':
		'No API key configured for the hosted DeepSeek API. Run "Yoke: Set API Key", or point yoke.baseUrl at a self-hosted endpoint.',
	...
	'vision.panel.title': 'Yoke Vision Proxy',
	'vision.panel.description':
	...
	// Request
	'request.toolsLimitExceeded':
		'This endpoint supports at most {0} functions in a single `tools` request, but this request contains {1}. Use VS Code Configure Tools to disable tools you rarely use. If the experimental tool-list stabilization setting is enabled, turn it off.',
	'request.preflightRoundLimitExceeded':
	...
	'notice.visionProxyMissing':
		'⚠️ Vision Proxy is unavailable. The model cannot see images. [Configure Vision Proxy]({0})',
	'notice.visionProxyFailure': '**⚠️ {0}**\\\n\\\n**{1} · {2}**',
	'notice.toolDrift':
		'⚠️ Tool list is unstable; cache hit rate may drop. [Learn more](https://github.com/emiluzelac/yoke/blob/main/docs/notices/tool-drift.en.md)',
	...
	'extension.deactivateFailed': 'Failed to prepare the Yoke provider for deactivate',
	'extension.welcomeFailed': 'Failed to show the Yoke welcome prompt',
	'extension.openRequestDumpsFolderFailed':
		'Failed to open request dumps folder. Run "Yoke: Show Logs" for details.',
};
